...
This problem is caused by JDK17 https://bugs.openjdk.java.net/browse/JDK-8270087[reporting some warnings to `stdout` instead of `stderr`, window="new"]. The easiest way to resolve this is to avoid the situation leading to the warning, or switching to a JDK version without this bug, like JDK11.
