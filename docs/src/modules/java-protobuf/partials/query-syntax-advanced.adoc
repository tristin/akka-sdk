...
To do this, create a class with a `ComponentId` annotation, and with a nested `View` component (a static member class) for each of the view tables. Each nested view table subscribes to one of the entities.
