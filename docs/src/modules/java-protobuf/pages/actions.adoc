...
By default, the generated constructor has an `ActionContext` parameter, but you can change this to accept other parameters.
...
To illustrate how you can use an Action as a Controller, we will build on top of the xref:proto.adoc[Key Value Entity Shopping Cart] example, adding a new Action to the existing shopping cart service.
...
In that previous example, we build a controller around the xref:value-entity.adoc[Key Value Entity Counter] and forwarded the incoming request
after modifying it.
