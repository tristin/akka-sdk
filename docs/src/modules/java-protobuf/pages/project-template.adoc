...
* A [.group-java]#*Maven archetype*# [.group-scala]#*giter8 template*# that generates the recommended project structure, a simple Counter service containing a Key Value Entity, and unit tests. A `README.md` explains what was created and how to work with the newly created service.
...
Kalix uses gRPC Protocol Buffers language to describe the service interface and the entity domain model. The archetype generates a `CounterService` API implemented as a https://docs.kalix.io/reference/glossary.html#value_entity[Key Value Entity]. The entity descriptors include:

- `src/main/proto/com/example/counter_api.proto` the service API to be used by clients
- `src/main/proto/com/example/counter_domain.proto` the domain model of the Key Value Entity's state
...
On the first build, the plugin creates a Key Value Entity implementation class where you implement the business logic for command handlers (e.g., `Counter`) .
...
For more details, see xref:java-protobuf:value-entity.adoc#_testing_the_entity[Testing a Key Value Entity].
