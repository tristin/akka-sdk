...
It will often be necessary to access the generated id from inside the component code. This can be done using the context, for instance for KeyValueEntity, use link:{attachmentsdir}/api/kalix/javasdk/EntityContext.html#entityId()[`EntityContext.entityId` {tab-icon}, window="new"] method.
...
<1> Indicates to the codegen that we want to generate a Key Value Entity.
<2> The package and name of our Key Value Entity.
...
The `shoppingcart_domain.proto` file specifies the state and messages for a Key Value Entity.
