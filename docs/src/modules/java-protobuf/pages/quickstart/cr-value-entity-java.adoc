...
[source,command line]
----
curl -OL https://raw.githubusercontent.com/lightbend/kalix-jvm-sdk/main/samples/protobuf-customer-registry-quickstart/pom.xml
----
...
. Add the service endpoint. The service endpoint is annotated with `kalix.codegen` indicating we want to generate a Key Value Entity for this service.
+
[source,proto,indent=0]
.src/main/proto/customer/api/customer_api.proto
----
...
The `customer_domain.proto` contains all the internal data objects (https://docs.kalix.io/reference/glossary.html#entity[Entities, window="new"]). The https://docs.kalix.io/reference/glossary.html#value_entity[Key Value Entity, window="new"] in this sample is a Key/Value store that stores only the latest updates.
...
* You can learn more about xref:java-protobuf:value-entity.adoc[Key Value Entities].
