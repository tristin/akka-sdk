...
This is done by passing an extra property (`akka.platform.dev-mode.service-port-mappings.customer-registry`) to its Kalix Runtime to let it create a mapping between the logical name *customer-registry* and the host and port where the *customer-registry* service is running.
...
[source, command line]
----
mvn -Dakka.platform.dev-mode.docker-compose-file=docker-compose-integration.yml kalix:runAll
----
--
...
[source, command line]
----
sbt -Dakka.platform.dev-mode.docker-compose-file=docker-compose-integration.yml runAll
----
--
