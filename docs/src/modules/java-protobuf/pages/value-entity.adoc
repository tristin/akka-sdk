...
NOTE: Our Key Value Entity example starts with the "Counter" service as included in the xref:project-template.adoc[project template].
...
<9> `value_entity` indicates that we want the codegen to generate a Key Value Entity for this service.
<10> `name` denotes the base name for the Key Value Entity, the code-generation will create initial sources `Counter`, `CounterTest` and `CounterIntegrationTest`. Once these files exist, they are not overwritten, so you can freely add logic to them.
...
<12> `state` points to the protobuf message representing the Key Value Entity's state which is kept by Kalix. Note, the package and name follow the definition in the domain.proto file.
...
<8> `value_entity` indicates that we want the codegen to generate a Key Value Entity for this service.
<9> `name` denotes the base name for the Key Value Entity, the code-generation will create initial sources `Counter`, `CounterTest` and `CounterIntegrationTest`. Once these files exist, they are not overwritten, so you can freely add logic to them.
...
<11> `state` points to the protobuf message representing the Key Value Entity's state which is kept by Kalix. Note, the package and name follow the definition in the domain.proto file.
...
We need to implement all methods our Key Value Entity offers as https://docs.kalix.io/reference/glossary.html#command_handler[_command handlers_].
...
NOTE: For Key Value Entities, modify the state and then trigger a save of that state in the returned `Effect`. The Java/Protobuf SDK has an `effects().updateState(newState)` method for this purpose. If you change the state but do not call `updateState` in the returned `Effect`, that state change is lost.
...
To make Kalix aware of the Key Value Entity, we need to register it with the service.
...
An example integration test class to start from is also generated for you. It uses [.group-java]#an `AkkaSdkTestkitExtension`# [.group-scala]#`AkkaSdkTestKit`# to start docker containers and interacts with the entity with an actual gRPC client.
