...
To demonstrate its functionality, let's consider an Ordering Service composed of a xref:value-entity.adoc[Key Value Entity] and an xref:actions.adoc[Action]. The Action will work as a xref:actions-as-controller.adoc[Controller] receiving calls and delegating to the Order Entity.
...
As said before, the `OrderEndpoint` will act as a controller intercepting incoming messages, running some logic and then calling the Order Entity.
...
----
<1> When the `OrderEndpoint` receives the expiration call, it immediately tries to cancel the Order. You use the `execute()` method to run it and this method returns a `CompletionStage`.
...
----
<1> When the `OrderEndpoint` receives the expiration call, it immediately tries to cancel the Order. You use the `execute()` method to run it. This method returns a `Future`.
