...
We now need to configure a route to use a secret. Routes can be created by following the instructions in xref:operations:services/invoke-service.adoc#_exposing_services_to_the_internet[exposing services to the internet].
...
Configuration client certificate validation can be done either with command line arguments or xref:operations:services/invoke-service.adoc#descriptor[route descriptors], though route descriptors are a little more powerful:
