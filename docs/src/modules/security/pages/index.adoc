...
Access to projects is managed by granting users roles. Different roles enable different capabilities when interacting with a project. To understand which roles exist and how to assign them, refer to the section xref:operations:manage-project-access.adoc[]. Project membership is managed via the `akka roles` commands.

To allow for scripted access to projects, Akka supports authentication tokens. For details see xref:operations:integrate-cicd.adoc[].

Akka also provides secret management for each project, typically used for passwords, login credentials, keys, etc. You can provide secrets to your services through environment variables. For details see xref:operations:secrets.adoc[] section.


== Organization level

Similarly to projects, access to organizations is managed by granting users roles that provide different capabilities. To understand which roles exist and how to assign them, consult the section xref:operations:manage-users.adoc[]. Organization membership is managed via the `akka organization invitations` and `akka organization users` commands.
