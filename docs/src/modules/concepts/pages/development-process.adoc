...
[#_create_a_project]
== Create a project
All services and applications start as a Java project. Akka has a Maven archetype that makes this easier. You will code your service in this project. See xref:java:build-your-first-application.adoc#[] for more details.
...
TIP: Kickstart a project using the xref:java:build-your-first-application.adoc[getting started guide].
...
Stateful services utilize components like xref:java:key-value-entities.adoc[Key Value Entities],
xref:java:event-sourced-entities.adoc[Event Sourced Entities], xref:java:workflows.adoc[Workflows], and
xref:java:views.adoc[Views], while stateless services focus on exposing functionality via xref:java:http-endpoints.adoc[HTTP Endpoints]. Typically, a stateful service is centered around one Entity type but may also include Endpoints and Views to expose or retrieve data.
