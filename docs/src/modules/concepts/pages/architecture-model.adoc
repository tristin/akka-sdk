...
When you want to _do something_, i.e. change state or take action, you implement methods in this layer with return type `Effect` or `ReadOnlyEffect`. The effects are signals to the runtime to persist your events: Akka will persist them for you and handle concurrent updates, scaling, and many other difficult tasks. This is a key part of what makes Akka so responsive and resilient. This model streamlines development and reduces the amount of ceremony you must perform to build Akka applications.
