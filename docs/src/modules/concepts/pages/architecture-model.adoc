...
The application layer is where Akka really comes into play as the glue between your domain code and the Runtime. Most classes in this layer will extend an Akka component class (i.e. something in the `akka.javasdk` package) which means it will be event-based and actor-based behind the scenes, though you never have to really think about that.

When you want to _do something_, i.e. change state or take action, you implement components methods returning an `Effect` or `ReadOnlyEffect`. An effect is a description of what the Runtime needs to do after the command is handled. You can think of it as a set of instructions you are passing to the Runtime, which will process the instructions on your behalf.

Each component type defines its own Effect API, which are a set of predefined operations that match the capabilities of that specific component. For example, event-sourced entities have an effect API that allows you to persist your events: Akka will persist them for you and handle concurrent updates, scaling, and many other difficult tasks. This is a key part of what makes Akka so responsive and resilient. This model streamlines development and reduces the amount of ceremony you must perform to build Akka applications.
