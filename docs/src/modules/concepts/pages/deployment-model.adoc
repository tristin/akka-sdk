...
=== Region
xref:operations:organizations/regions.adoc[] are specific Akka clusters running in a specific place. Regions are designed to run independently for greater availability. They are also designed to replicate data asynchronously between eachother. Every Akka xref:concepts:deployment-model.adoc#_project[project] specifies a list of one or more regions in which it runs.
