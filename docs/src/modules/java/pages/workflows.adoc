...
:sample-url: https://github.com/lightbend/akka-javasdk/tree/main/samples/java-spring-transfer-workflow
...
* *type id* is a unique identifier to the workflow. To define the workflow type id, the workflow class must be annotated with `@ComponentId` and have a unique and stable identifier assigned.
...
<2> Make sure to annotate such class with `@ComponentId` and pass a unique identifier for this workflow type.
...
NOTE: The `@Id` value `transferId` must match a path parameter (i.e. `transferId`) and such value needs to be unique per workflow. On the other hand, the `@ComponentId` value `transfer` is common for all instances of this workflow but must be stable - cannot be changed after a production deploy - and unique across the different workflow types.
...
A full error handling and compensation sample is available https://github.com/lightbend/kalix-jvm-sdk/tree/main/samples/spring-transfer-workflow-compensation[here, {tab-icon}, window="new"]. Run `TransferWorkflowIntegrationTest` and examine the logs from the application.
