...
Consider an example of a Customer Registry service with a `customer` Key Value Entity. When `customer` state changes, the entire state is emitted as a value change. Value changes update any associated Views.
...
As well as a Key Value Entity service `CustomerEntity.java` that will produce the state changes consumed by the View. You can consult xref:value-entity#entity-behavior[Key Value Entity] documentation on how to create such an entity if in need.
...
<2> `@Consume` annotation is at method level rather than at class level.
...
Key Value Entities can be xref:value-entity#deleting_state[deleted]. We can update our view model based on that fact with an additional flag `handleDeletes` for the subscription.
...
----
<1> `@Consume` annotation with `handleDeletes=true`.
...
NOTE: When using @Consume on a class level, `handleDeletes=true` will also work. Kalix will automatically delete the View state.
...
An Event Sourced entity can emit many types of events. You need to define a method for each event type. They return an `Effect`, which describes next processing actions, such as updating the view state.
