...
* xref:java:http-endpoints.adoc[REST Endpoints]
...
* xref:java:consuming-producing.adoc[Consuming and Producing]
...
The persistent Kalix entities (xref:java:event-sourced-entities.adoc[Event Sourced Entity] and xref:java:key-value-entities.adoc[Key Value Entity]) represent the command-model in the CQRS pattern. They receive `Commands` and mutate data in their own transaction.
