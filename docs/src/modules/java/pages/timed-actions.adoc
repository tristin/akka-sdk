...
To demonstrate its functionality, let's consider an Ordering Service composed of a xref:key-value-entities.adoc[Key Value Entity] and an xref:actions.adoc[Action]. The Action will work as a xref:actions-as-controller.adoc[Controller] receiving calls and delegating to the Order Entity.
...
As said before, the `OrderEndpoint` will act as a controller ahead of the Order Entity, receiving incoming messages, running some logic and then calling the Order Entity as needed.
