...
TIP: If you are hearing about `KeyValueEntity` for the first time, be sure to visit xref:key-value-entities.adoc[] before continuing.
...
<2> Set up the Kalix infrastructure by extending `TestKitSupport`.
