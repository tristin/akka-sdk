...
As an example, we will use the https://github.com/lightbend/kalix-jvm-sdk/tree/main/samples/spring-eventsourced-customer-registry[`customer-registry`] sample. 
...
For that we will use two of our existing samples: https://github.com/lightbend/kalix-jvm-sdk/tree/main/samples/spring-eventsourced-customer-registry[`customer-registry`] and https://github.com/lightbend/kalix-jvm-sdk/tree/main/samples/spring-eventsourced-customer-registry-subscriber[`customer-registry-subscriber`].
...
On the other hand, in the *customer-registry-subscriber* we will use port 9001 to avoid port conflicts with the *customer-registry* service. This port is configured in `akka.javasdk.dev-mode.http-port` property in `pom.xml`.
...
In the previous example, we run two services independently with `mvn compile exec:java`. Each time, the service is started together with a Kalix Runtime running in a Docker container.
...
[source,command line]
----
mvn -Dakka.javasdk.dev-mode.docker-compose-file=docker-compose-integration.yml kalix:runAll
----
--
...
Note that the configuration is exactly the same except that now we are running one single `mvn compile exec:java` command and the `docker-compose` file we are using contains all the dependencies required by *customer-registry-subscriber*.
