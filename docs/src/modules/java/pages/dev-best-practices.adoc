...
Each Akka Service consists of one or more Components and is packaged and deployed as a unit. Akka services are deployed to Akka Projects. Thus, when you couple multiple business concerns by packaging them in the same service, even under separate xref:java:dev-best-practices.adoc#_Bounded_context[bounded contexts], you limit the runtime's ability to make the most efficient decisions to scale up or down.
