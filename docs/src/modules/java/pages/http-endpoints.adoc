...
A service does rarely contain only internal components but also publishes an API for the surrounding world to
interact with it.

HTTP Endpoint components makes it possible to conveniently define such APIs accepting and responding in JSON,
...
.src/main/java/com/example/api/ExampleEndpoint.java
[source,java]
----
...
[source,java]
----
include::example$eventsourced-customer-registry-subscriber/src/main/java/customer/api/CustomerRegistryEndpoint.java[tag=cross-service-call]
----
