...
https://docs.kalix.io/reference/glossary.html#value_entity[Key Value Entities] persist the full state on every change. Only the latest state is stored, so we don’t have access to any of the history of changes, unlike event sourced storage.

NOTE: Key Value Entities currently doesn't support multi-region replication.

NOTE: Akka _Key Value Entities_ have nothing in common with the domain-driven design concept of _Value Objects_. The _Value_ in the name refers to directly modifying a value for the entity's state - in contrast to https://docs.kalix.io/reference/glossary.html#event_sourced_entity[Event-sourced Entities] that persist events and the entity state is derived from them.
...
[source,java]
----
include::example$valueentity-counter/src/main/java/com/example/application/CounterEntity.java[tags=declarations]
----
...
----
include::example$valueentity-counter/src/main/java/com/example/application/CounterEntity.java[tags=behaviour]
----
...
[source,java,indent=0]
----
include::example$valueentity-counter/src/main/java/com/example/application/CounterEntity.java[tags=query]
----
...
[source,java,indent=0]
----
include::example$valueentity-counter/src/main/java/com/example/application/CounterEntity.java[tags=delete]
----
...
[source,java,indent=0]
----
include::example$valueentity-counter/src/test/java/com/example/CounterTest.java[tags=example]
----
...
[source,java,indent=0]
----
include::example$valueentity-counter/src/it/java/com/example/CounterIntegrationTest.java[tags=sample-it]
----
