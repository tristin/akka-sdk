...
.{sample-base-url}/event-sourced-counter/src/main/java/counter/application/CounterEntity.java[CounterEntity.java]
----
include::example$event-sourced-counter/src/main/java/counter/application/CounterEntity.java[tag=increaseWithError]
----
...
[source,java,indent=0]
.{sample-base-url}/event-sourced-counter/src/main/java/counter/api/CounterEndpoint.java[CounterEndpoint.java]
----
include::example$event-sourced-counter/src/main/java/counter/api/CounterEndpoint.java[tag=increaseWithError]
----
...
.{sample-base-url}/event-sourced-counter/src/main/java/counter/application/CounterEntity.java[CounterEntity.java]
----
include::example$event-sourced-counter/src/main/java/counter/application/CounterEntity.java[tag=increaseWithResult]
----
...
.{sample-base-url}/event-sourced-counter/src/main/java/counter/api/CounterEndpoint.java[CounterEndpoint.java]
----
include::example$event-sourced-counter/src/main/java/counter/api/CounterEndpoint.java[tag=increaseWithResult]
----
...
All unexpected exception thrown be the user code are transformed into an HTTP 500 error. When running the service locally in dev mode, a stack strace is will be a part of the HTTP response. In production, this information is hidden, to not leak internal details about the service to a client. The client will receive a non-descriptive message with a correlation ID, like below.
...
That same correlation ID `2c74bdfb-3130-464c-8852-cf9c3c2180ad` is included in the log entry for the error as an MDC value with the key `correlationID`. This makes it possible to find the specific error in the logs using `kalix logs` or by querying your configured logging backend for the service.
