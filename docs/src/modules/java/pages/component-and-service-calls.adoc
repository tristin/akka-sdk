...
<4> Potentially adapt the response rather than returning it as is.

The component client can call command handlers on Event Sourced Entities, Key Value Entities and workflows, query methods.

The component client is only available for injection in the following types of components: Endpoints, Workflows, Consumers and Timed Actions.
...
[source,java,indent=0]
.src/main/java/com/example/callanotherservice/DelegatingServiceEndpoint.java
----
...
The HTTP client provider client is only available for injection in the following types of components: Endpoints, Workflows, Consumers and Timed Actions.
