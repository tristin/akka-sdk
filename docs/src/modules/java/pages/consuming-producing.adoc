...
image:ROOT:consumer.png[Consumer,width=100,float=left]  Consuming and producing allow stream based interaction between services and also with the outside world. Consuming or publishing a stream of events is a common Microservices pattern. The source of events can be the journal of an Event Sourced Entity, the Key Value Entity state changes, or a message broker topic for asynchronous messaging between services and other systems.
