...
:sample-url: https://github.com/lightbend/akka-javasdk/tree/main/samples/java-spring-eventsourced-shopping-cart
...
<3> Make sure to annotate such class with `@ComponentId` and pass a unique identifier for this entity type.
<4> Use Spring's RequestMapping annotation to define the route to your entity.


NOTE: The `@Id` value `cartId` must match a path parameter (i.e. `cartId`) and such value needs to be unique per entity. On the other hand, the `@ComponentId` value `shopping-cart` is common for all instances of this entity but must be stable - cannot be changed after a production deploy - and unique across the different entity types.
...
----
<1> Note the test class must extend `AkkaSdkTestKitSupport`.
