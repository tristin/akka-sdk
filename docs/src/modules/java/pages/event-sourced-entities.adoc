...
:sample-url: https://github.com/lightbend/akka-javasdk/tree/main/samples/eventsourced-shopping-cart
...
[sidebar]
Event Sourced Entities offer strong consistency guarantees. Akka distributes Entities across every instance of a stateful service deployment--at any given time, each Entity lives on exactly one instance. If a command for an Entity arrives to an instance not hosting that Entity, the command is forwarded by the Akka Runtime to the one that contains that particular Entity. This forwarding is done transparently, your code does not need to know. Because each Entity lives on exactly one instance, messages can be handled sequentially. Hence, there are no concurrency concerns relating to Event Sourced Entities, each Entity handles one message at a time.
...
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/domain/ShoppingCart.java[tag=domain]
----
...
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/domain/ShoppingCartEvent.java[tag=events]
----
...
[source,java]
.src/main/java/com/example/shoppingcart/domain/application/ShoppingCartEntity.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java[tag=class]
----
...
<2> Make sure to annotate such class with `@ComponentId` and pass a unique identifier for this entity type.
...
.src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java[tag=addItem]
----
...
.src/main/java/com/example/shoppingcart/domain/ShoppingCart.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/domain/ShoppingCart.java[tag=itemAdded]
----
...
.src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java[tag=getCart]
----
...
----
include::example$eventsourced-shopping-cart/src/main/resources/application.conf[tag=snapshot-every]
----
...
.src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java[tag=checkout]
----
...
It is not allowed to persist more events after the entity has been "marked" as deleted. You can still handle read requests of the entity until it has been completely removed.
...
.src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java
----
include::example$eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/application/ShoppingCartEntity.java[tag=read-only]
----
...
The replication mode is defined when deploying the service for all entities of the service or different modes for certain entity types. For each such configuration it's also possible to define which regions that participate in the replication.
...
[source,java]
----
include::example$eventsourced-shopping-cart/src/test/java/com/example/shoppingcart/domain/ShoppingCartTest.java[]
----
...
[source,java,indent=0]
----
include::example$eventsourced-shopping-cart/src/it/java/com/example/IntegrationTest.java[tag=sample-it]
----
...
[source]
----
/akka/v1.0/entity/<component id>/<entity id>/getCart
----
