...
* A number of xref::samples:index.adoc[complete examples] let you start with working code.
...
* The Akka *Maven parent pom* that defines dependencies and helps you run your application locally and package for deployment.

== Prerequisites

* An https://console.akka.io/register[Akka account]
* Java {java-version}, we rely on https://adoptium.net/marketplace/[Eclipse Adoptium]
* https://maven.apache.org/install.html[Apache Maven, window="new"] version {minimum_maven_version} or later (https://maven.apache.org/docs/history.html[Maven version history, window="new"])
...
The `Counter` domain object is a Java record:

.src/main/java/com/example/domain/Counter.java
[source,java,indent=0]
----
include::example$key-value-counter/src/main/java/com/example/domain/Counter.java[tags=domain]
----

To expose a HTTP API of the counter we add an endpoint:

.src/main/java/com/example/api/CounterEndpoint.java
[source,java,indent=0]
----
include::example$key-value-counter/src/main/java/com/example/api/CounterEndpoint.java[tags=endpoint]
----

The project also includes a dependency on the *Akka SDK Test Kit*. Please consult the specific documentation page
