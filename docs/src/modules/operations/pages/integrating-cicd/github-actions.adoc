...
Use the Akka https://github.com/lightbend/setup-akka-action[setup-akka-action, window="new"] GitHub Action to use GitHub Actions with your Akka project. The action supports commands for installing, authenticating, and invoking the Akka CLI. Releases are tracked https://github.com/lightbend/setup-akka-action/releases[on the GitHub releases page]. The action is present in the https://github.com/marketplace/actions/setup-akka-cli[GitHub marketplace].
...
The https://github.com/akka/akka-cicd-github-actions[akka/akka-cicd-github-actions, window="new"] repository contains an example of a workflow that automatically builds, tests, and deploys a Node.js service to Akka.
...
To use the https://github.com/akka/akka-cicd-github-actions[akka/akka-cicd-github-actions, window="new"] example, copy the workflow files and save the required variables as GitHub https://docs.github.com/en/actions/reference/encrypted-secrets#creating-encrypted-secrets-for-a-repository[secrets, window="new"]:
