...
To use the https://github.com/akka/akka-cicd-github-actions[akka/akka-cicd-github-actions] example, copy the workflow files and save the required variables as GitHub https://docs.github.com/en/actions/reference/encrypted-secrets#creating-encrypted-secrets-for-a-repository[secrets {tab-icon}, window="new"]:

. Copy the workflow `.yml` files from the example repository https://github.com/akka/akka-cicd-github-actions/tree/main/.github/workflows[.github/workflows/ {tab-icon}, window="new"] directory into your project repository `.github/workflow` directory.
