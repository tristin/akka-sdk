...
The `akka` command can also start a graphical client, which embeds https://github.com/fullstorydev/grpcui[grpcui {tab-icon}, window="new-tab"] in the proxy, allowing you to easily explore your services and build and make requests on them. To enable it, use the `--grpcui` flag:
...
- xref:reference:akka-cli/akka_routes.adoc#_see_also[`akka routes` commands]
- xref:reference:akka-cli/akka_services.adoc#_see_also[`akka services` commands]
