...
The _Organization_ is the root of the management hierarchy in Akka. It is container for all xref:operations:projects.adoc[] where services are deployed. The organization services as a context for users within it. When working in https://console.akka.io[Akka Console] or the Akka CLI you will operate in this context. If you want to switch organizations you must specify the organization context to which you want to switch.

An organization has access to a set of xref:operations:regions.adoc[regions]. Projects created in the organization will be located in one or more of these regions.
