...
The generation of traces is disabled by default. To enable it you need to set xref:reference:descriptor-reference.adoc#_akkaservicespec[telemetry/tracing/enabled] to `true`. Like the following: 
...
Once this is set, Akka will start generating https://opentelemetry.io/docs/concepts/observability-primer/#spans[spans] for the following components: Event Sourced Entities, Value Entities, Actions, and Views. In addition, a View or an Action that subscribes to an Entity will receive the traces from that Entity and create a span related to the previous trace. 

Another possible use is to add a `traceparent` header when calling your Akka endpoint. In this case Akka will propagate this trace parent as the root of the subsequent interaction with a Akka component. Linking each Akka call to that external trace parent. 

==== Setting sampling

By default, Akka doesn't filter your traces. You'll get 100% of the traces produced by your services, but this is usually too much data. That's why Akka allows you to adjust this percentage according to your needs. Based on the TraceID, if the sampling percentage is less than 100%, the traces will be filtered and won't be exported to the endpoint you have configured. By default, this is the Control Tower found in your Akka Console.

This filtering is known as  https://opentelemetry.io/docs/concepts/sampling/#head-sampling[head sampling {tab-icon}, window="new"]. To configure it, you need to set your observability descriptor as follows:
...
With this configuration, 2% of the traces are sent while the 98% are filtered.
...
In addition, the OTLP exporter supports <<TLS configuration>> and <<Custom headers,custom headers>>. A full reference of configuration options is available in xref:reference:descriptor-reference.adoc#_akkaobservabilityotlp[the reference documentation].
...
In addition, the Prometheus exporter supports <<TLS configuration>> and <<Custom headers,custom headers>>. A full reference of configuration options is available in xref:reference:descriptor-reference.adoc#_akkaobservabilityprometheuswrite[the reference documentation].

=== Splunk HEC

The Splunk HTTP Event Collector protocol is supported for exporting both metrics and logs. It is used to export to the Splunk platform. It needs an endpoint and a splunk token. The Splunk token must be configured as a xref:operations:secrets.adoc[Akka secret], which then gets referenced from the observability configuration.
...
In addition, the Splunk HEC exporter supports <<TLS configuration>>. A full reference of configuration options is available in xref:reference:descriptor-reference.adoc#_akkaobservabilitysplunkhec[the reference documentation].
...
----
. Place the key file in a Akka secret, we'll call the secret `gcp-credentials`. The key for key file must be `key.json`.
+
[source,shellscript]
----
...
Now that you have configured service account, granted it the necessary roles, created a service account key and placed it into a Akka secret, you can now configure your observability configuration to export to Google Cloud:
...
To specify a custom CA to verify server certificates, use the server CA property, pointing to a Akka CA secret:
...
To specify a client certificate to use to authenticate with a remote server, you can use the client certificate property, pointing to a Akka TLS secret:
...
- xref:reference:akka-cli/akka_projects_observability.adoc#_see_also[`akka project observability` commands]
