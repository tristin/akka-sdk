...
Once this is set, Akka will start generating https://opentelemetry.io/docs/concepts/observability-primer/#spans[spans, window="new"] for the following components: Event Sourced Entities, Key Value Entities, Consumers, and Views. In addition, a View or a Consumer that consumes from an Entity will receive the traces from that Entity and create a span related to the previous trace.
