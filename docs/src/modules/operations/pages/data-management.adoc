...
6. Place the access key created in step 5 in a Akka secret.
...
Instructions for creating a bucket in AWS can be found https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html[here].

==== Create a user

For the purposes of these instructions, we're going to call the user `my-akka-user`. Instructions for creating users in AWS can be found https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html[here].
...
Instructions for creating IAM policies in AWS can be found https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_create-console.html[here].

==== Attach IAM policy to user

Having created the policy, you can now attach the policy to the user you created earlier. Instructions for doing that can be found https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_manage-attach-detach.html[here].

==== Create an access key

Now that the user has the right permissions, you need to create an access key for that user. Instructions for doing this can be found https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html[here]. Copy the access key ID and secret access key for use in the next step.
...
Instructions for creating a bucket in GCS can be found https://cloud.google.com/storage/docs/creating-buckets[here].

==== Create a service account

For the purposes of these instructions, we're going to call the user `my-akka-service-account`. Instructions for creating service accounts in GCP can be found https://cloud.google.com/iam/docs/service-accounts-create[here].

==== Granting access

For exports, you will need to grant the service account the Storage Object Admin role to the bucket (`roles/storage.objectAdmin`). For imports, you will need to grant the service account the Storage Object Viewer role to the bucket (`roles/storage.objectViewer`). Instructions for managing IAM policies for buckets can be fonud https://cloud.google.com/storage/docs/access-control/using-iam-permissions[here].

==== Create the service account key

You will need to create a JSON service account key for the `my-akka-service-account` for Akka to use to access the bucket. This can be done by following https://cloud.google.com/iam/docs/keys-create-delete[these instructions]. Once the key is created, place it in a file called `key.json`.
