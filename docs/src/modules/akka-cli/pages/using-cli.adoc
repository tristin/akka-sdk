...
The Akka command-line interface (CLI) complements the browser-based `Console` UI and enables deployment and configuration operations directly from the command line. This page provides an overview of common commands. For a full reference of all available commands, see the xref:reference:akka-cli/index.adoc[reference] page.
