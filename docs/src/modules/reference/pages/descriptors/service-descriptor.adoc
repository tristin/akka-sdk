...
Replication configuration for stateful components, such as Entities and Workflows, in the service. See xref:operations:projects/multi-region.adoc#_selecting_primary_for_stateful_components[Selecting primary for stateful components] for more information and considerations before changing.
...
|*PrimarySelectionMode*   |string     a|Selection of primary can be `static`, `dynamic` and `none`.

* `static` - one region is defined as the primary for the project, and all stateful component instances will use that region as primary
* `dynamic` - the primary is selected by each individual component instance based on where the first event is persisted
