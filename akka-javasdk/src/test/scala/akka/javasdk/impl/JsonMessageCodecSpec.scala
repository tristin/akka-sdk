...
import JsonMessageCodecSpec.Cat
import JsonMessageCodecSpec.Dog
import JsonMessageCodecSpec.SimpleClass
import JsonMessageCodecSpec.SimpleClassUpdated
...
class JsonMessageCodecSpec extends AnyWordSpec with Matchers {
  ...
  "The JsonMessageCodec" should {
    ...
    "default to FQCN for typeUrl (java)" in {
      ...
      encoded.getTypeUrl shouldBe jsonTypeUrlWith("akka.javasdk.impl.JsonMessageCodecSpec$SimpleClass")
    }
    ...
    "add version number to typeUrl" in {
      ...
      encoded.getTypeUrl shouldBe jsonTypeUrlWith("akka.javasdk.impl.JsonMessageCodecSpec$SimpleClassUpdated#1")
    }
    ...
    "default to FQCN for typeUrl (scala)" in {
      ...
      encoded.typeUrl shouldBe jsonTypeUrlWith("akka.javasdk.impl.JsonMessageCodecSpec$SimpleClass")
    }
...

    {
      ...
      "fail when using the same TypeName" in {
        ...
        exception.getMessage shouldBe "Collision with existing existing mapping class akka.javasdk.impl.JsonMessageCodecSpec$AnnotatedWithTypeName$Elephant -> elephant. The same type name can't be used for other class class akka.javasdk.impl.JsonMessageCodecSpec$AnnotatedWithTypeName$IndianElephant"
      }
...

    {
      ...
      "default to FQCN  if TypeName has empty string (java)" in {
        ...
        encodedLion.getTypeUrl shouldBe jsonTypeUrlWith(
          "akka.javasdk.impl.JsonMessageCodecSpec$AnnotatedWithEmptyTypeName$Lion")

        val encodedElephant = messageCodec.encodeJava(Elephant("Dumbo", 1))
        encodedElephant.getTypeUrl shouldBe jsonTypeUrlWith(
          "akka.javasdk.impl.JsonMessageCodecSpec$AnnotatedWithEmptyTypeName$Elephant")
      }
