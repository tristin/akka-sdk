...
class ProxyInfoHolder extends Extension {
   ...
   * INTERNAL API
   */
  private[akka] def overridePort(port: Int): Unit =
    _proxyPort.set(port)

  /**
   * INTERNAL API
   */
  private[akka] def overrideProxyHost(host: String): Unit =
    _proxyHostname.set(host)

  private[akka] def overrideTracingCollectorEndpoint(endpoint: String): Unit =
