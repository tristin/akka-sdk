...
AKKA_GRPC_PROTOC_FULL=$(sbt -no-colors "print java-sdk/protocVersion" 1>/tmp/protoc-version.txt 2>/dev/null)
# the tail distance is different locally and on CI
AKKA_GRPC_PROTOC=$(cat /tmp/protoc-version.txt | tail -n 1 | head -n 1 | xargs)
...
if [ $(echo "$POM_PROTOC_VERSIONS" | wc -l) -ne 1 ]; then
  ...
  echo "DEBUG: full Akka gRPC version text: \"\"\"$(cat /tmp/protoc-version.txt)\"\"\""
  false
fi
