...
package akka.platform.testkit.keyvalueentity

import akka.stream.scaladsl.Source
import akka.stream.testkit.TestPublisher
import akka.stream.testkit.scaladsl.TestSink
import kalix.protocol.value_entity.{ ValueEntitiesClient, ValueEntityStreamIn, ValueEntityStreamOut }
import akka.platform.testkit.TestProtocol.TestProtocolContext

final class TestKeyValueEntityProtocol(context: TestProtocolContext) {
  private val client = ValueEntitiesClient(context.clientSettings)(context.system)

  def connect(): TestKeyValueEntityProtocol.Connection = new TestKeyValueEntityProtocol.Connection(client, context)

  def terminate(): Unit = client.close()
}

object TestKeyValueEntityProtocol {
